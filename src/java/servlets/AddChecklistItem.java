...
public class AddChecklistItem extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) 
        ...
        // Handle FormData by reading the request body manually
        if (request.getContentType() != null && request.getContentType().contains("multipart/form-data")) {
            try {
                ...
                System.out.println("DEBUG: FormData body: " + body.toString());
                
                // Parse FormData manually (simple approach for text fields)
                String[] lines = body.toString().split("\n");
                String itemName = null;
                String branchId = null;
                
                for (String l : lines) {
                    if (l.contains("name=\"itemName\"")) {
                        // Find the next line which contains the actual value
                        int index = java.util.Arrays.asList(lines).indexOf(l);
                        if (index + 1 < lines.length) {
                            itemName = lines[index + 1].trim();
                            if (itemName.isEmpty()) itemName = null;
                        }
                    } else if (l.contains("name=\"itemBranch\"")) {
                        // Find the next line which contains the actual value
                        int index = java.util.Arrays.asList(lines).indexOf(l);
                        if (index + 1 < lines.length) {
                            branchId = lines[index + 1].trim();
                            if (branchId.isEmpty()) branchId = null;
                        }
                    }
                }
                
                System.out.println("DEBUG: FormData parsed - itemName='" + itemName + "', branchId='" + branchId + "'");
                
                if (itemName == null || itemName.trim().isEmpty() || branchId == null || branchId.trim().isEmpty()) {
                    System.out.println("DEBUG: FormData validation failed");
                    out.print("{\"success\": false, \"message\": \"Item name and branch are required. [FormData: itemName=" + itemName + ", branchId=" + branchId + "]\"}");
                    return;
                }
                
                // Continue with the rest of the logic using itemName and branchId
                processChecklistItem(itemName, branchId, out);
                return;
                
            } catch (Exception e) {
                System.out.println("DEBUG: FormData parsing error: " + e.getMessage());
                e.printStackTrace();
            }
        }
    ...
    private void processChecklistItem(String itemName, String branchId, PrintWriter out) throws IOException {
        Connection con = null;
        try {
            con = DatabaseConfig.getConnection();
